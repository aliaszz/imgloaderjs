...
function ImgLoader(settings) {
  ...
  let getResourceSort = function (orderedTags) {
  ...
  let onProgress = function (resource, statusType) {
    ...
    let numResourceTags = resource.tags.length;

    // fire callbacks for interested listeners
    for (let i = 0, numListeners = progressListeners.length; i < numListeners; i++) {
      let listener = progressListeners[i],
        shouldCall;
  ...
  let log = (this.log = function (showAll) {
    ...
    let elapsedSeconds = Math.round((+new Date() - timeStarted) / 1000);
    window.console.log('ImgLoader elapsed: ' + elapsedSeconds + ' sec');
    ...
    for (let i = 0, len = entries.length; i < len; i++) {
      ...
      let message = 'ImgLoader: #' + i + ' ' + entry.resource.getName();
      switch (entry.status) {
...
function ImgLoaderImage(url, tags, priority) {
  let self = this,
    loader = null;
  ...
  // called by ImgLoader to check status of image (fallback in case
  ...
  // called by ImgLoader when it is no longer waiting
...
// add a convenience method to ImgLoader for adding an image
ImgLoader.prototype.addImage = function (url, tags, priority) {
  let imageLoader = new ImgLoaderImage(url, tags, priority);
